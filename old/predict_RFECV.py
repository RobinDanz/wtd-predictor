...
def train_xgboost_model(X, y):
    ...
    # Sélection récursive avec validation croisée
    rfecv = RFECV(estimator=model, step=1, cv=KFold(5), scoring=scorer)
    rfecv.fit(X_train, y_train)

    print("Nombre optimal de features :", rfecv.n_features_)
    print("Features sélectionnées :", X.columns[rfecv.support_])

    X_train = X_train.drop(list(X.columns[rfecv.support_]), axis=1)
    X_test = X_test.drop(list(X.columns[rfecv.support_]), axis=1)
